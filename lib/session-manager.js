...
class SessionManager extends EventEmitter {
  ...
  sendInput(sessionId, data) {
    ...
    // These come from xterm.js responding to queries in the buffer, not actual user input
    const filtered = data
      .replace(/\x1b\[\?[0-9;]*c/g, '')  // Primary DA response (\x1b[?1;2c)
      .replace(/\x1b\[>[0-9;]*c/g, '')   // Secondary DA response
      .replace(/\x1b\[[0-9]+;[0-9]+R/g, '') // Cursor position report
      .replace(/\x1b\[[\?]?[0-9;]*n/g, '') // Device status report

    // Skip if only terminal responses (no actual input)
    if (!filtered) return;
    ...
    if (!session.firstInputCaptured) {
      session.inputBuffer = (session.inputBuffer || '') + filtered;
    ...
    session.statusDetector.onInput(filtered);

    session.pty.write(filtered);
    session.lastActivity = new Date().toISOString();
  }

  async _captureFirstInput(session, input) {
    // Debug: log raw input for troubleshooting
    // console.log('Raw input:', JSON.stringify(input), 'hex:', Buffer.from(input).toString('hex'));

    // Clean up input - strip ANSI escape sequences
    let cleaned = input
      .replace(/\x1b\[[?!>]?[0-9;:]*[a-zA-Z~]/g, '') // CSI sequences
      .replace(/\x1b\[[0-9;]*[~$^@]/g, '')           // Function key sequences
      .replace(/\x1b\][^\x07\x1b]*(?:\x07|\x1b\\)/g, '') // OSC sequences
      .replace(/\x1b[PX^_][^\x1b]*\x1b\\/g, '')      // DCS/SOS/PM/APC sequences
      .replace(/\x1b./g, '')                         // Any remaining escape + char
      .replace(/[\x00-\x1F\x7F]/g, '')               // Control characters

    // Only keep printable characters and common punctuation
    // This filters out any garbage that slipped through
    cleaned = cleaned
      .split('')
      .filter(c => /[\x20-\x7E\u00A0-\uFFFF]/.test(c)) // Printable ASCII + Unicode
      .join('')
      .replace(/\s+/g, ' ')
      .trim();
